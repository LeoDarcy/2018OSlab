...
#define SYS_SEMA_init 201
#define SYS_SEMA_wait 202
#define SYS_SEMA_post 203
#define SYS_SEMA_destroy 204
...
void sys_sema_init(struct TrapFrame *tf)
{
	//优先注意偏移量的问题
	int offset=0;
	while(offset<MAX_PCB_NUM && (&pcb[offset])!=pcb_run)
		offset++;
	if(offset>=MAX_PCB_NUM)
	{
		putChar('*');putChar('s');putChar('m');putChar('i');putChar('t');putChar('*');		
		assert(0);
	}
	
	sem_t *sem=(sem_t*)(tf->ebx+offset*PRO_MEM);
	//得到空闲的sema结构
	int sem_index=0;
	while(sem_index<MAX_SEMA_NUM && semaphore[sem_index].state!=SEMA_DEAD)
		sem_index++;
	if(sem_index>=MAX_SEMA_NUM)
	{
		putChar('*');putChar('s');putChar('m');putChar('i');putChar('t');putChar('*');		
		assert(0);
	}
	semaphore[sem_index].state=SEMA_USED;
	semaphore[sem_index].value=tf->ecx;
	*sem=sem_index;
	tf->eax=1;
}

void sys_sema_wait(struct TrapFrame *tf)
{
	//优先注意偏移量的问题
	int offset=0;
	while(offset<MAX_PCB_NUM && (&pcb[offset])!=pcb_run)
	...
	if(offset>=MAX_PCB_NUM)
	{
		putChar('*');putChar('s');putChar('m');putChar('w');putChar('a');putChar('*');		
		assert(0);
	}
	sem_t *sem=(sem_t*)(tf->ebx+offset*PRO_MEM);
	int sema_index=*sem;
	/*if(semaphore[sema_index].state!=SEMA_USED)
	{
		putChar('*');putChar('s');putChar('m');putChar('w');putChar('a');putChar('*');		
		assert(0);
	}*/
	semaphore[sema_index].value--;
	tf->eax=0;
	//阻塞判断 为了方便，根本不需要把阻塞进程放入pcb_blocked中，只需要放在sema_pcb_blocked即可
	if(semaphore[sema_index].value<0)
	{
		semaphore[sema_index].sema_pcb_blocked[offset]=SEMA_BLOCKED;
		pcb_run->state=PROCESS_BLOCKED;
		pcb_run=pcb_run->next;
		process_switch();
	}
...
void sys_sema_post(struct TrapFrame *tf)
{
	//优先注意偏移量的问题
	int offset=0;
	while(offset<MAX_PCB_NUM && (&pcb[offset])!=pcb_run)
	...
	if(offset>=MAX_PCB_NUM)
	{
		putChar('*');putChar('s');putChar('m');putChar('p');putChar('o');putChar('*');		
		assert(0);
	}
	sem_t *sem=(sem_t*)(tf->ebx+offset*PRO_MEM);
	int sema_index=*sem;
	...
	{
		int sema_bl=0;
		while(sema_bl<MAX_PCB_NUM && semaphore[sema_index].sema_pcb_blocked[sema_bl]!=SEMA_BLOCKED)
			sema_bl++;
		if(sema_bl>=MAX_PCB_NUM)
		{
			putChar('*');putChar('s');putChar('m');putChar('p');putChar('o');putChar('*');		
			assert(0);
		}
		pcb[sema_bl].state=PROCESS_RUNNABLE;
		pcb[sema_bl].next=pcb_run->next;
		pcb_run->next=&pcb[sema_bl];
		process_switch();
	}
}

void sys_sema_destroy(struct TrapFrame *tf)
{
	//优先注意偏移量的问题
	int offset=0;
	while(offset<MAX_PCB_NUM && (&pcb[offset])!=pcb_run)
	...
	if(offset>=MAX_PCB_NUM)
	{
		putChar('*');putChar('s');putChar('m');putChar('d');putChar('e');putChar('*');		
		assert(0);
	}
	sem_t *sem=(sem_t*)(tf->ebx+offset*PRO_MEM);
	int sema_index=*sem;
	...
	semaphore[sema_index].state=SEMA_DEAD;
	tf->eax=0;
}
...
void syscallHandle(struct TrapFrame *tf) {
	...
	switch( tf->eax)
	{
		...
		case SYS_SEMA_init:sys_sema_init(tf);break;
		case SYS_SEMA_wait:sys_sema_wait(tf);break;
		case SYS_SEMA_post:sys_sema_post(tf);break;
		case SYS_SEMA_destroy:sys_sema_destroy(tf);break;
		default:assert(0);
	}

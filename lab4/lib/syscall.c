...
{
	return process_syscall(SYS_SEMA_init,(uint32_t)sem,value,1);
}

void sem_wait(sem_t *sem)
{
	process_syscall(SYS_SEMA_wait,(uint32_t)sem,1,1);
}

void sem_post(sem_t *sem)
{
	process_syscall(SYS_SEMA_post,(uint32_t)sem,1,1);
}

void sem_destroy(sem_t *sem)
{
	process_syscall(SYS_SEMA_destroy,(uint32_t)sem,1,1);
}
